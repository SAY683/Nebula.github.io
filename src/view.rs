...
use crate::special_type::NebulaError;
...
impl GUI {
	...
	///#必须panic[unwrap]使用 pub fn numerical_treatment<GX: Sized>(f: &str, e: anyhow::Result<GX, anyhow::Error>) -> GX
	...
	///#pub fn error_display<GX: Sized>(f: &str, e: Result<GX, NebulaError>)
	pub fn error_display<GX: Sized>(f: &str, e: Result<GX, NebulaError>) {
		e.unwrap_or_else(|x| {
			eprintln!(
				"{}",
